...
static esp_err_t execute_cmd(sgp40_t *dev, uint16_t cmd, uint32_t timeout_ms, uint16_t *out_data, size_t out_words, uint16_t *in_data, size_t in_words)
{
	CHECK_ARG(dev);
...
esp_err_t sgp40_init(sgp40_t *dev)
{
	...
	ESP_LOGI(TAG, "SGP40 S/N: 0x%04x%04x%04x, featureset 0x%04x", dev->serial[0], dev->serial[1], dev->serial[2], dev->featureset);
...
static void sgp40_task(void *pvParameter)
{
	sgp40_t sgp;
	...
	while (1)
	{
		...
		ESP_LOGI(TAG, "%.2f °C, %.2f %%, VOC index: %" PRIi32 ", Air is [%s]", temperature, humidity, voc_index, voc_index_name(voc_index));
...
//    ESP_LOGI(TAG, "Humidity sensor initilalized");
    ...
    xTaskCreatePinnedToCore(&sgp40_task, "sgp40_task", SGP40_TASK_STACK_SIZE, NULL, SGP40_TASK_PRIORITY, NULL, SGP40_TASK_CORE_ID);
