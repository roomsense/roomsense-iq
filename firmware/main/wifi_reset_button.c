...
{
	// Notify the button task by releasing the semaphore
	xSemaphoreGiveFromISR(wifi_reset_semphore, NULL);
}

/**
 * Wifi reset button task reacts to SW2 button event by sending a message
...
void wifi_reset_button_task(void *pvParam)
{
	for (;;)
	{
		if (xSemaphoreTake(wifi_reset_semphore, portMAX_DELAY) ==  pdTRUE)
		{
			...
			//wait 5 sec and check out the pin again
			vTaskDelay(5000 / portTICK_PERIOD_MS);

			if(gpio_get_level(WIFI_RESET_BUTTON) == 0) {
